...
class ManageCSWConnectionsDialog( QDialog, Ui_ManageCSWConnectionsDialog ):
  ...
  def selectFile( self ):
    if self.dialogMode == 0:
      self.fileName = QFileDialog.getSaveFileName( self, self.tr( "Save connections" ), ".", self.tr( "XML files (*.xml *.XML)" ) )
    else:
      self.fileName = QFileDialog.getOpenFileName( self, self.tr( "Load connections" ), ".", self.tr( "XML files (*.xml *XML)" ) )
